...
Dicitonaries I use relatively frequently, especially for storing data/metadata - and if you have a dictionary of lists, it is easy to convert it to a pandas dataframe.
...
if 'Gapdh' in expressed_genes:
    print('Gapdh is an expressed gene')
...
print(wt1_df.head())
wt1_df.drop(['GeneSum'], axis=1, inplace=True) # removing the new column
...
```{python}
#| code-fold: true
#| cold-summary: "Reading in the dataset:"
...
```{python}
#| code-fold: true
#| cold-summary: "Melting the dataset:"
...
plt.figure(figsize=(6,4))
sns.violinplot(x='group', y='expression', data=merged_df[merged_df.gene == 'Gene2'], palette='muted')
plt.title('Gene1 Expression by Group')
plt.show()

plt.figure(figsize=(6,4))
sns.violinplot(x='group', y='expression', data=merged_df[merged_df.gene == 'Gene7'], palette='muted')
plt.title('Gene3 Expression by Group')
plt.show()
...
```{python}
# Fake fold change and p-value data (for illustration)
import numpy as np

# Create fake log2 fold change and p-value data
volcano_data = pd.DataFrame({
    'gene': ['Gene0', 'Gene1', 'Gene2', 'Gene3', 'Gene4', 'Gene5', 'Gene6', 'Gene7', 'Gene8', 'Gene9'],
    'log2FC': np.random.randn(10),  # Random fold changes
    'pval': np.random.rand(10)  # Random p-values
})
...
# Create a binary response for KO vs WT group
merged_df['group_binary'] = merged_df['group'].apply(lambda x: 1 if x == 'KO' else 0)

# Let's use Gene expression as a continuous response
...
# Assume 'df' has two conditions: 'KO' and 'WT'
...
# Categorize gene expression into 'high' or 'low' based on median expression
...
# Perform ANOVA on expression values grouped by the 'group' column
...
# Example of a dataframe 'df' with gene expression data (assuming you already have this)
# We assume `group` is the column that has 'KO' and 'WT' labels and expression data per gene
...
# Iterate over each gene (rows of the dataframe)
for g in merged_df.gene.unique():
    # Get expression data for the gene in both KO and WT groups
...
# Now apply multiple testing correction to the p-values
# Here we use the Benjamini-Hochberg (FDR) method, you could use Bonferroni if you prefer
...
I will talk more about this when showing my re-do of the analyses.
...
# Assuming normalize function is defined here
...
iPython allows you to interactively - few poitns on why its useful.
