...
Below I will show you how to set up a Python enviornment - the first step is choosing between environment managers: `venv` and `conda`.\

Working in an environment will help you develop your code in a controlled setting, where you will use a package manager to install the packages that are required to run your program. How to install packages is also shown in the coding block below.
...
For me, I prefer using venv with pip - but many people like conda which was designed for data science and can do a bit more than just being a package installer. It can even manage software stacks beyond Python. conda comes packaged with Anaconda or miniconda, which basically provide you with many of the packages required for scientific analyses - it is typically slower than just using venv and pip sometimes to install packages.
...
## TIP 3: Everything in Python is an object, which has its own attributes and methods!
...
Two objects that are important for Python programming are functions and classes - which also exist in R. Below, I show how they are typically formatted.
...
In the above block, I demonstrate the use of docstrings - so that others and most importantly you, can remember what the function/class is. I have used the 'numpy' version of docstrings, the most population option is the google format. Docstrings can be accessed by using `help(my_function_name)`
...
Side note: you can remove a variable in python using `del varname`.
...
Side note: there are also generator, dictionary and set comprehensions - read more about it [here](https://www.geeksforgeeks.org/comprehensions-in-python/).
...
# dictionaries are good to store most object types e.g. you could have a list of genes epxressed in key cells
genes_expressed = {
    "Cell1": ['gene1', 'gene2'],
...
# dictionaries are really useful for storing data - you could have multiple dicts e.g. genes_mutated, gene_lengths and you can acess teh data for a particular sample with the same key e.g. genes_mutated['sample1'] or gene_lengths['sample1'].
...
Below, I show some ways to index and slice a list, for you reference.
...
Sometimes you want to do an operation, but there is an expected error that might pop up - normally an error will cause your program to exit - you might not want this behaviour. With errors you expect, you can catch the error and do something other than exiting - this is achieved by using `try except`./
...
# remember you can use dir(pd.DataFrame) and help() to better understand the attributes/methods of this object type.
...
## TIP 9: You maye need numpy to do your classical mathematic functions
...
Seaborn is typically improted as `sns`.
...
I use seaborn for the scatter plot, and hten layer on top of it using `matplotlib.pyplot` (`plt`).
```{python}
...
# fig will be a plotly.graph_objects.Figure
```
...
I will show you some examples, there is no actually reason to run these tests - it's just to show you how they can be done.
